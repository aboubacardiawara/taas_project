...
let ex_beta_mult : pterm = App (Abs ("x", Add (Var "x", N 1)), N 2)
(*4. 2 applications imbriquées: Expression initiale : (λx.x (λy.y))  (λz.z) *)
let ex_beta_nested : pterm = App (App (Abs ("x", App (Var "x", Abs ("y", Var "y"))), Abs ("z", Var "z")), Abs ("w", Var "w"))
(*5 variable libre dans une abstraction*)
let ex_free_var : pterm = App (Abs ("x", App (Var "x",  Var "y")), Var "z")